...
DEVNULL = open(os.devnull, "w") # ??? I don't fucking stats
...
def formula_to_image(formula):
    ...
    name = hashlib.sha1(formula.encode('utf-8')).hexdigest()[:15] # generate unique name
    ret = []
    for rend_name, rend_setup in RENDERING_SETUPS.items():
        full_path = name+"_"+rend_name  # construct path for formula
        if len(rend_setup) > 2 and rend_setup[2](full_path):    # check if not done(rend_setup = 3 & third element returns true)
        ...
        if code != 0:   # check if error (code = 0 means successful execution in UNIX-like systems)
            os.system("rm -rf "+full_path+"*")  # remove files
            return None
